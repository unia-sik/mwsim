/*
 * This file is part of MWSim.
 *
 * Copyright 2014 University of Augsburg
 *
 * MWSim is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * MWSim is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with MWSim.  If not, see <http://www.gnu.org/licenses/>.
 */

/**
 * $Id: context.h 1643 2016-02-18 07:11:10Z klugeflo $
 * MOSSCA WCET Simulator
 * @file context.h
 * @brief Execution Context
 * @author Florian Kluge <kluge@informatik.uni-augsburg.de>
 */

#ifndef _EXEC_CONTEXT_H
#define _EXEC_CONTEXT_H 1

#include <repr/command.h>
#include <repr/program.h>

#include <tools/variablestore.h>

namespace mwsim {


  class Context {
  public:

    //static const std::string VAR_NODE_ID;
    static const char* VAR_NODE_ID;

    Context(const Program &_program, const VariableStore *vsParent = NULL);
    Context(const Program &_program, NodeId nodeId, const VariableStore *vsParent = NULL);
    ~Context();

    Command *getCurrentCommand();
    bool advancePC();
    VariableStore *getVariables();

  private:
    VariableStore *variables;
    Program program;
    Program::const_iterator pc;

  };

} // NS

#endif /* !_EXEC_CONTEXT_H */
